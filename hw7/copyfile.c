...
Then, the program will copy the contents of the 1st file into the 2nd
*/
...
int main()
{
  ...
  FILE *pFileR, *pFileW; //File pointers to read and write to
  ...
  printf("Simple example: myfile.txt myfilecopy.txt\n");
  printf("Example using paths: ../lab2/echoline.c ~/echoline.c\n");
  ...
  if ((pFileW = fopen(input.tok[1], "w")) == NULL){ //Open file for writing
    ...
    while (getline(&bufferF, &buffLenF, pFileR) >= 0){ //Read lines until end of file
      fputs(bufferF, pFileW); //Copy those lines (with nullbyte terminations added) into the new file.
    }
    fclose(pFileR); fclose(pFileW); //Close both files
  }
