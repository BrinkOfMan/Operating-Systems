...
a) Is the final value of val consistent among all the runs, or does it vary?
If that value varies, can you explain that?

The final answer is consistently 10 (hooray!)

b) Do you see any evidence of a race condition in the output from your multiple runs?
If so, can you explain what went wrong?

Originally, yes. There were a couple trials where printf states "adding/subtracting" before doing the opposite of what it says it will do.
After adding the printf statement into the mutex lock, this error was resolved.
